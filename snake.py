...
class Snake:
    def __init__(self):
        ...
        # corner images
        self.corner_up_from_left = pygame.image.load('assets/images/body_tl.png').convert_alpha()
        self.corner_up_from_right = pygame.image.load('assets/images/body_tr.png').convert_alpha()
        self.corner_down_from_left = pygame.image.load('assets/images/body_bl.png').convert_alpha()
        self.corner_down_from_right = pygame.image.load('assets/images/body_br.png').convert_alpha()
    ...
    def draw_snake(self):
        for index, section in enumerate(self.body):
            snake_section = pygame.Rect(section.x * cell_size,
                                        ...
                                        cell_size)
            if index == 0:
                screen.blit(self.head_image_dict.get(tuple(self.direction)), snake_section)
            elif index == len(self.body) - 1:
                tail_direction = self.body[-2] - self.body[index]
                screen.blit(self.tail_image_dict.get(tuple(tail_direction)), snake_section)
            else:
                ...
                corner_image = self.corner_image_dict.get(tuple(previous_pos_diff + next_pos_diff))
                if corner_image is not None:
                    screen.blit(corner_image, snake_section)
                else:
                    screen.blit(self.body_image_dict.get(tuple(previous_pos_diff)), snake_section)
...
class Main:
    ...
    def draw_score(self):
        ...
        score_surface = self.font.render(score_text, True, pygame.Color('black'))
...
game_over_message = font.render(GAME_OVER_MSG, True, pygame.Color(175, 215, 70))
...
while True:
    ...
    # screen.blit(close_surface, (board_width - cell_size, cell_size))
    screen.blit(close_surface, close_surface.get_rect(topright = (screen.get_rect().topright)))
    # game-over screen layer
    if not main_game.can_move:
        game_over_dialog.fill(pygame.Color('black'))
        # center the message
        message_rect = game_over_message.get_rect(center = (game_over_dialog.get_rect().center))
